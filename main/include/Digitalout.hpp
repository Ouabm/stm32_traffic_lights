...
namespace cadmium
{
    ...
    public:
        ...
        DigitalOutput(const std::string &id, GPIO_TypeDef *selectedPort, GPIO_TypeDef *selectedPort2, GPIO_InitTypeDef *selectedPin1, GPIO_InitTypeDef *selectedPin2, GPIO_InitTypeDef *selectedPin3) : Atomic<DigitalOutputState>(id, DigitalOutputState()), port(selectedPort), port2(selectedPort2), pin1(*selectedPin1), pin2(*selectedPin2), pin3(*selectedPin3), R(32, 0, 0), Y(30, 16, 0), G(0, 32, 0)
        {
            in = addInPort<RGB>("in");
        ...
        void output(const DigitalOutputState &state) const override
        {
            ...
            if (state.output == G)
            {
                HAL_GPIO_WritePin(port, pin1.Pin, GPIO_PIN_SET);
